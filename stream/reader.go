...
type Reader struct {
	...
	out         ProgressWriter // Where to send progress bar to
	size        int64
	current     int64
	lastUpdate  int64
	id          string
...
func NewProgressReader(in io.ReadCloser, out ProgressWriter, size int64, id, action string) *Reader {
	return &Reader{
		in:          in,
		out:         out,
		size:        size,
...
func (p *Reader) Close() error {
	if p.current < p.size {
		// Complete the displayed progress bar when closing before EOF.
		p.current = p.size
